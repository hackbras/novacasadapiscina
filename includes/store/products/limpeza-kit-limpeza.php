<div class="store">
    <?php
        include "includes/store/product-menu.php";
    ?>

    <section class="store__products">
        <h1 class="store__products--title">Produtos</h1>
        <h2 class="store__products--subtitle">Kit Limpeza</h2>
        <div class="store__products--articles">
            <article>
                <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">                    
                    <input type="submit" value="COMPRAR">
                 </a>
            </article>

            <article>
                <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">                    
                    <input type="submit" value="COMPRAR">
                </a>
            </article>

            <article>
               <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">    
                    <input type="submit" value="COMPRAR">
                </a>
            </article>

            <article>
               <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20%</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">                    
                    <input type="submit" value="COMPRAR">
                </a>
            </article>

            <article>
                <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">                    
                    <input type="submit" value="COMPRAR">
                </a>
            </article>

            <article>
                <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">                    
                    <input type="submit" value="COMPRAR">
                </a>
            </article>

            <article>
               <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">                    
                    <input type="submit" value="COMPRAR">
                </a>
            </article>

            <article>
               <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details"  href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">
                    <input type="submit" value="COMPRAR">
                </a>
            </article>

            <article>
               <span class="store__product--freight">FRETE GRÁTIS</span>
                <span class="store__product--discount">-20</span>
                <figure class="store__product--description">
                    <img src="assets/img/produtos/tipos_de_piscinas/angra.png" alt="">
                    <figcaption>Nome do produto</figcaption>
                </figure>
                <span class="store__product--values-or-status">por: R$ xx.xxx,00 ou até 5x de R$ xx,00</span>
                <a class="store__product--details" href="index.php?pagina=includes-information/product/product-description.php">
                    <input type="submit" value="DETALHES">
                </a>
                <a class="store__product--to-buy" href="index.php?pagina=includes/store/product-store-car.php">
                    <input type="submit" value="COMPRAR">
                </a>
            </article>
        </div>
    </section>

    <section class="store__pagination">
        <div class="store__pagination--options">
            <a href=""href=""> << </a>
            <a href="">1</a>
            <a href="">2</a>
            <a href="">3</a>
            <a href="">4</a>
            <a href="">5</a>
            <a href=""> >> </a>
        </div>       
    </section>
</div>